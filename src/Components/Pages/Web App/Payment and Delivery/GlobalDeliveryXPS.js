...
//   // );
// }
// export default GlobalDeliveryXPS;
...
function GlobalDeliveryXPS({
  ...
  var CalculateShippingPricev2 = async (item,deliveryStatus) => {
    ...
    if (countryId === "226" || countryId === 226) {
      ...
      setDeliveryBy("usps_intl");
      SetDeliveryStatus("usps_intl");
      }
  ...
  let DeliveryhandleChange = async ({ currentTarget: input }) => {
    ...
    if (input.name === "ExpirationDate") {
      ...
      if(parseInt(input.value.length)==4){
        ...
        var response = await BanglaBazarApi.post(
          `${Endpoint}/api/usps/global-price-plan`,
          { 
     ...
     {
      let StateID=JSON.parse(input.value).StateID
        getCities(StateID);
  ...
  var savePaymentDetails = async () => {
    ...
    if (CountrySelect === "16" || CountrySelect === 16) {
      proceedPayment()
  ...
  var proceedPayment = async () => {
    ...
    if (parseInt(dd["CountryID"]) == 226  && parseInt(CountrySelect) == 16){
      ...
      if(CheckEmpty(dd["DeliveryZipCode"]) || dd["DeliveryZipCode"].length !== 4){
        return firetoast("Please provide a 4 digits valid zipcode", "default-error");
      }
                      ...
                      <span className="text-default">
                       ...
                       { console.log(! DeliveryDetails["DeliveryCity"], !DeliveryDetails["DeliveryState"])}
                        { ! DeliveryDetails["DeliveryCity"] || !DeliveryDetails["DeliveryState"] ? <><div
                         className="text-danger mt-4" style = {{fontSize: "15px"}}>Please enter delivery state and delivery city first</div></>:  <>Valid 4 digits ZipCode is required for shipping price calculation 
                    </>
