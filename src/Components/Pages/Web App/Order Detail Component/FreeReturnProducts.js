...
function FreeReturnProducts() {
        ...
        try {
            ...
            if (response) {
                ...
                for (let i = 0; i < length; i++) {
                    ...
                    console.log(FreeProductReturn, "FreeProductReturn   ")
                    if (FreeProductReturn === "Y") {
    ...
    console.log(freeReturnOrders, "freeReturnOrders")
    ...
    let calculateSubtotal = () => {
        ...
        for (let i = 0; i < currentOrders.length; i++) {
            ...
            if (currentOrders[i]["checked"]) {
                ...
                // }
                if (currentOrders[i]["FreeProductReturn"] === "N" && currentOrders[i]["VendorPaymentStatus"] === "N") {
                    _subTotal -= parseFloat(currentOrders[i]["ItemsShippingHandling"]);
                                ...
                                {Orders.map((item, index) => (
                                    item.FreeProductReturn === "Y" ?
                                        <div className="row mt-2 align-items-center" key={index} >
                                                ...
                                                <div className="form-check form-check-inline">
                                                    <input
                                                        ...
                                                        onChange={(e) => {
                                                            ...
                                                            setOrders(temp);
                                                            calculateSubtotal();
                                                            let temp_array = [...productArray];
                                                            if (e.target.checked) {
                                                                temp_array.push(item.ProductID)
                                                                setProductChecked(true);
                                                            }
                                                            else {
                                                                const index = temp_array.indexOf(item.ProductID);
                                            ...
                                            <div className="col-2">
                                                {item["FreeProductReturn"] === "Y" && (
                                                    <span className="text-orange">Free Shipping</span>
                                                )}
                                            </div>
                                            <div>
                                                {item.ReturnReason && (
...
export default FreeReturnProducts;
