...
function BusinessEmailVerificationModal(props) {
  const { BusinessEmail, StoreEmail } = useContext(VendorRegistrationContext);
  const { emailVerify, setEmailVerify, setEmailVerified } = props;
  const toggle = () => setEmailVerify(!emailVerify);
    ...
    if (otp.length < 0) {
      ...
      try {
        ...
        var response = await axios.post(
          `${Endpoint}/api/store-management/otp-verification`,
          form
        );
  ...
  var resendOtp1 = async () => {
    try {
      ...
      form.append("EmailAddress", email);
      var response = await axios.post(
        `${Endpoint}/api/store-management/email-verification`,
        form
      );
      if (response.data.status) {
  ...
  var resendOtp = async () => {
    try {
      ...
      form.append("EmailAddress", email);
      var response = await axios.post(
        `${Endpoint}/api/store-management/resend-otp`,
        form
      );
      if (response.data.status) {
        return toast.info("Otp sent to your email", {
  ...
  return (
    <div>
      <Modal
        isOpen={emailVerify}
        toggle={() => setEmailVerify(!emailVerify)}
        size="lg"
        backdrop="static"
      >
...
export default BusinessEmailVerificationModal;
