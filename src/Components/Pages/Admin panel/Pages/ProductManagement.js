...
function ProductManagement() {
  ...
  var getProducts = async () => {
    ...
    data["sort"] = sort;
    try {
      if (getRoles().includes("Vendor")) {
        response = await BanglaBazarApi.post(
          `${Endpoint}/api/product/get-productDetailsPaginated/${CurrentUser.UserID}`,
          data
        );
        ...
        console.log("else")
        history.push("/panel/product-management/all");
      }
      // } else {
      //   response = await BanglaBazarApi.post(
        ...
        <div>
          {getRoles().includes("Admin") && (
            <button
              ...
              onClick={() => history.push("/panel/product-management/all")}
            >
              <i className="fas fa-shopping-bag"></i> View All Products
            </button>
          )}
          <button
            className="btn-default"
